...
graph (`public/m6-bakeoff-graph.json`) ships in the repo; regenerate it after
analyzer changes with:
...
- [Design methodology (adhere to this for UI work)](docs/DESIGN.md)
...
4. Finish hardening local AI setup: a guided readiness stepper and streaming
   Ask on top of the now-separate generation/embedding readiness checks.
...
- Split the 3.6k-line `src/App.tsx` into feature components.
- Replace grep-based UI/accessibility smokes with a driven-browser smoke.
- Stream AI Ask/summary output instead of buffering behind a fixed timeout.
