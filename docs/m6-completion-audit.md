...
This audit checks the local v1/M6 continuation goal against current repo evidence. The official benchmark suite was cloned into `.cache/benchmarks/COBOL-Legacy-Benchmark-Suite` for local validation, but it remains untracked. Linux packaging is validated in WSL; GitHub Actions now validates unsigned Windows/Linux package builds and uploads unsigned bundle artifacts for QA, but signed Windows installer readiness is not claimed.

## Requirement Evidence

| Requirement | Current evidence | Status |
| --- | --- | --- |
| User-facing Dependencies UI exists without parser-internal coupling | `src/App.tsx` renders lineage, depends-on, used-by, and relationship detail panels from `GraphDocument` nodes/edges only. It uses edge types such as `reads`, `writes`, `moves-to`, `queries`, `links`, `uses-dd`, and node types such as `data-item`, `dataset`, `db2-table`, `cics-command`, `jcl-dd`. | Done |
...
| Packaging implications are explicit | `npm run m6:packaging-readiness` reports sidecar/JDK sizes, startup smoke timings, WSL Linux prerequisites, Windows host prerequisites, and a repeatable `packagedDebSmoke` when a `.deb` bundle exists. Current Linux readiness is true. `npm run tauri build` produced `.deb`, `.rpm`, and `.AppImage` bundles under `src-tauri/target/release/bundle/`. The Linux `.deb` includes the production analyzer at `usr/lib/Cobolens/binaries/cobolens-analyze` and the bundled sample at `usr/lib/Cobolens/samples/mini-bank/`. The packaged analyzer parsed the packaged sample with 4 parsed files, 25 nodes, 27 edges, and 0 parse errors. `npm run desktop:packaged-smoke` launches the AppImage under WSLg and verifies that the packaged GUI stays alive with WebKit/GStreamer runtime complete. The GitHub Actions package workflow validates unsigned Windows/Linux Tauri bundle builds and uploads OS-specific unsigned bundle artifacts; signed Windows release installers remain unclaimed. | Linux packaging validated; unsigned Windows CI added |
| V1 readiness evidence can be rerun from one command | `npm run v1:readiness` now runs the required `m6:verify` suite, then records optional local gates when prerequisites exist. On this WSL machine it passed `m6:verify`, the cached COBOL Legacy Benchmark Suite, local Ollama readiness with `llama3.2:1b`, local Ollama grounded AI summary smoke, local Ollama grounded Ask smoke with guarded cited fallback, and the packaged Linux AppImage GUI smoke. | Current local readiness sweep passed |
...
- Linux/Tauri packaging is validated for the Rust production sidecar, including
  ...
  GitHub Actions validates unsigned Windows/Linux package builds and uploads
  unsigned bundle artifacts for QA. Signed Windows/Tauri release packaging has
  not been validated.
...
2. Validate signed Windows/Tauri packaging before a public Windows installer.
